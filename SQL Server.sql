...
CREATE TABLE ALUNO(
	IDALUNO INT PRIMARY KEY IDENTITY,/*IDENTITY = AUTOINCREMENT*/
...
CREATE TABLE ENDERECO(
	...
	BAIRRO VARCHAR(30),
	UF CHAR(2) NOT NULL
	CHECK (UF IN('RJ','SP','MG')),/*CONSTRAINT DENTRO DA TABELA*/
	ID_ALUNO INT UNIQUE
)
GO
...
SP_COLUMNS ALUNO /*SP - STORE PROCEDURES: EQUIVALE A DESC TABLE DO MYSQL*/
GO

SP_HELP ALUNO /*EQUIVALE A UM DESC TABLE MAIS DETALHADO*/
GO

/* INSERINDO DADOS */

/*NÃO É PRECISO PASSAR "NULL" NA COLUNA REFERENTE A PK IDENTITY(AUTOINCREMENT)*/
...
DELETE FROM ENDERECO WHERE ID_ALUNO = 4
GO
...
/*QUERY DE TODOS OS ALUNOS INCLUINDO OS SEM TELEFONE*/
SELECT A.NOME, T.TIPO, T.NUMERO, E.BAIRRO, E.UF
...
SELECT NOME, GETDATE() AS DIA_HORA FROM ALUNO /*TRAZ O DIA E HORA DA SOLICITAÇÃO*/
GO

/* 1 PASSO */

SELECT NOME, DATEDIFF(DAY,NASCIMENTO,GETDATE()) /*DIFERENÇA EM DIAS ENTRE A DATA ATUAL E A DATA DE NASCIMENTO*/
FROM ALUNO
GO
...
SELECT NOME, DATEDIFF(DAY,NASCIMENTO,GETDATE()) IDADE/*NÃO É RECOMENDADO NÃO UTILIZAR O "AS"*/
FROM ALUNO
GO

/* 3 PASSO - RETORNO EM INTEIRO + OPER MATEMATICA (RETORNA UM INTEIRO) */
...
/* DATENAME - TRAZ O NOME DA PARTE DA DATA EM QUESTAO - (RETORNA UMA STRING) */
...
/* DATEPART - MESMA FUNÇÃO DO DATENAME MAS O RETORNO É UM INTEIRO */

SELECT NOME, DATEPART(MONTH,NASCIMENTO) AS "MÊS(INTEIRO)", 
	   DATENAME(MONTH, NASCIMENTO) AS "MÊS(STRING)"
FROM ALUNO
GO

/* DATEADD - RETORNA UMA DATA SOMANDO A OUTRA DATA */

SELECT DATEADD(DAY,365,GETDATE())/*PEGA A DATA ATUAL E SOMA EM DIAS O INTERVALO(Nº DE DIAS) DESEJADO* E RETORNA UMA DATA*/
GO

SELECT DATEADD(YEAR,10,GETDATE())/*PEGA A DATA ATUAL E SOMA EM ANOS O INTERVALO(Nº DE ANOS) DESEJADO E RETORNA UMA DATA*/
GO

/* CONVERSAO DE DADOS (CONVERSÃO IMPLICITA)*/
...
SELECT 'CURSO DE BANCO DE DADOS' + '1'/* = CURSO DE BANCO DE DADOS1*/
GO

SELECT 'CURSO DE BANCO DE DADOS' + 1/* ERRO */
GO
/*O PROGRAMA TENTA FORÇA A CONVERÇÃO DE STRING EM NUMERICO POR ISSO O PRIMEIRO EXEMPLO EXECUTOU*/

/* FUNCOES DE CONVERSAO DE DADOS (CONVERSÃO EXPLICITA)*/

SELECT CAST('1' AS INT) + CAST('1' AS INT)/*CONVERTE EM INTEIRO*/
GO
...
/*Falha ao converter o varchar valor '/' para o tipo de dados int, isso porque o
programa forçou essa converção pois identificou a variável dia como numérica e o 
usou '+' como operador lógico */
...
/*FOI CONVERTIDO A DATA EM STRING E ASSIM O PROGRAMA LEU O SINAL '+' COMO CONCATENADOR DE STRING*/

/*CHARINDEX - RETORNA UM INTEIRO 
CONTAGEM DEFAULT - INICIA EM 01
*/

/*CHARINDEX(O QUE, ONDE, A PARTIR DE) -> RETORNA UM INTEIRO*/

SELECT NOME, CHARINDEX('A',NOME) AS INDICE
FROM ALUNO
GO /*RETORNOU A POSIÇÃO DA LETRA 'A' NOS NOMES DA COLUNA 'NOME'*/

SELECT NOME, CHARINDEX('A',NOME,2) AS INDICE
FROM ALUNO
GO /*RETORNOU A POSIÇÃO DA LETRA 'A' NOS NOMES A PARTIR DO 2ºINDICE(2ªLETRA) DA COLUNA 'NOME'*/
...
WITH
(
	FIRSTROW = 2,/*DESCONSIDERA 1ª LINHA POIS SÃO OS TITULOS DA COLUNA*/
	DATAFILETYPE = 'char',/*ARQUIVOS DE CARACTERES*/
	FIELDTERMINATOR = '\t',/*DELIMITADOR DE COLUNAS, NESTE CASO É O 'TAB'*/
	ROWTERMINATOR = '\n'/*DELIMITADOR DE LINHAS, NESTE CASO É O 'ENTER'*/
...
/*
	BLOCO DE PROGRAMAÇÃO NOMEADO
		CREATE - BLOCO DE PROGRAMAÇÃO NOMEADO QUE SERÁ ARMAZENADO NO BANCO
		PODENDO SER EXECUTADO SEMPRE QUE SOLICITADO
		(
			 CREATE...
			 ON...
			 FOR...
...
ON DBO.PRODUTOS /*DBO(DATABASE OWNER) - DIVISOR LÓGICO DE BANCO DE DADOS(SCHEMA)*/
FOR UPDATE
AS
		...
		--SEGUNDO BLOCO /*VALORES VINDOS DE FUNÇÕES OU VALORES LITERAIS DEVEM SER ATRIBUIDOS COM O COMANDO SET*/
		SET @DATA = GETDATE()
...
/*ALTERAÇÕES PARA QUE A TRIGGER SÓ ATUALIZE A TABELA HISTÓRICO QUANDO OCORRER ATUALIZAÇÃO DO PRECO*/

CREATE TRIGGER TRG_ATUALIZA_PRECO
ON DBO.PRODUTOS
FOR UPDATE AS
IF UPDATE(PRECO)/*LINGUAGEM TSQL-SO SERÁ RODADO O IF SE O COMANDO FOR REFERENTE AO PREÇO*/
/*
TODO CÓDIGO ACIMA DO BEGIN É OBRIGATÓRIO EM BLOCOS NOMEADOS
*/
BEGIN
...
/*
	DECLARE - BLOCO DE PROGRAMAÇÃO ANÔNIMO
	SÓ É EXECUTADO UMA VEZ E NÃO É ARMAZENADO EM BANCO (MUITO USADO EM TESTE)
*/
...
DECLARE
		...
		SET @RESULTADO = (SELECT 50 + 50)
		INSERT INTO RESULTADO VALUES(@RESULTADO)
		PRINT 'VALOR INSERIDO NA TABELA: ' + CAST(@RESULTADO AS VARCHAR)/*SEM O CAST DA ERRADO POIS O RESULTADO É INTEIRO E PRECISA SER CONVERTIDO PARA VARCHAR*/
		GO
		

/* TRIGGER SIMPLIFICADA UPDATE */

CREATE TABLE EMPREGADO(
	IDEMP INT PRIMARY KEY,
	NOME VARCHAR(30),
	SALARIO MONEY,/*POR PADRÃO SÃO 2 CASAS DECIMAIS; NO NUMERIC PODE DETERMINAR AS CASAS DECIMAIS*/
	IDGERENTE INT
)
GO
...
INSERT INTO SALARIO_RANGE VALUES(3000.00,6000.00)/*PISO E TETO (SALARIO)*/
GO
...
AS 
	...
	IF(@ATUALSAL < @MINSAL)
	BEGIN
			RAISERROR('SALARIO MENOR QUE O PISO',16,1)/*MOSTRA MENSAGEM DE ERRO*/
			ROLLBACK TRANSACTION /*LOTE ANULADO*/
	END
...
/*
	AO INSERIR UMA INFORMAÇÃO ELA PRIMEIRO VAI PARA O 'LOG' E DEPOIS PARA DADOS.
	-QUANDO HÁ O ROLLBACK, ELA É ANULADA NO 'LOG' E NÃO ENTRA NOS DADOS.
	DIFERENTE DE QUANDO ALGO É EXCLUÍDO, POIS NESSE CASO HÁ INFORMAÇÃO JÁ FOI ARMAZENADA NOS DADOS
*/
...
SP_HELPTEXT TG_RANGE /*PROCEDURE(SP) - RETORNA A ESTRUTURA DA TRIGGER*/
GO
...
CREATE TABLE TELEFONE(
	...
	TIPO CHAR(3) NOT NULL CHECK ( TIPO IN('CEL','COM')),/*EQUIVALE AO ENUM DO MySQL*/
...
/*
	@ INDICA UM VARIÁVEL LOCAL
	@@ INDICA UMA VARIÁVEL GLOBAL
*/

CREATE PROC CONTA @NUM1 INT, @NUM2 INT /*PROCEDURE QUE RECEBE DOIS PARÂMETROS*/
AS
...
/* TRAZER OS TELEFONES DE ACORDO COM O TIPO(CEL, RES, COM) PASSADO POR PARÂMETRO*/
...
/*
	UMA PROCEDURE SEMPRE PEDE PARÂMETROS DE ENTRADA(@) MAS TAMBÉM
PODE TER PARÂMENTRO DE SAÍDA(OUTPUT), QUE DEVEM SER ARMAZENADO NUMA
VARIÁVEL(@) E É IMPRIMIDO NA TELA
...
GO /*RETORNA A QUANTIDADE DE NUMEROS ARMAZENADOS DO TIPO 'CEL'*/

DECLARE @SAIDA INT
EXEC GETTIPO 'CEL', @SAIDA OUTPUT
SELECT @SAIDA AS 'QUANTIDADE'
GO /*VERSÃO RESUMIDA DA DE CIMA*/
...
AS
	...
	= @@IDENTITY) /*ADICIONANDO VALOR A UMA VARÁVEL*/
...
--CONVERT(TIPO, VARIÁVEL, FORMATO): USADO MAIS QUANDO SE TRATA DE DATA
...
BEGIN
		...
		PRINT 'VALOR DE DATA: ' + CONVERT(VARCHAR, @V_DATA, 121) /*121= DATA(ANO/MêS/DIA) E HORÁRIO(H:MIN:SEG:MSEG)*/
		PRINT 'VALOR DE DATA: ' + CONVERT(VARCHAR, @V_DATA, 120) /*120= DATA(ANO/MêS/DIA) E HORÁRIO(H:MIN:SEG:MSEG)*/
		PRINT 'VALOR DE DATA: ' + CONVERT(VARCHAR, @V_DATA, 105) /*105= DATA(DIA/MÊS/ANO)*/
END
GO
...
/*O CASE NÃO PRECISA NECESSARIAMENTE ESTAR DENTRO DE UM BLOCO DE EXECUÇÃO*/
...
/*CRIANDO UM BLOCO NOMEADO(PROCEDURE) DO EXEMPLO ACIMA*/

CREATE PROCEDURE TRUE_FALSE @NUMERO INT
AS
	BEGIN 
		IF @NUMERO = 5 
			PRINT 'O VALOR É VERDADEIRO'
...
--FIZ ESSE BLOCO SOZINHA E TÁ CERTO!!!!!!
--ARRASEI
...
AS
	BEGIN 
		IF @NUMERO % 2 = 0 
			PRINT CAST(@NUMERO AS VARCHAR) + ' -> O VALOR É PAR'
		ELSE
			PRINT CAST(@NUMERO AS VARCHAR) + ' -> O VALOR É IMPAR'
	END
GO
